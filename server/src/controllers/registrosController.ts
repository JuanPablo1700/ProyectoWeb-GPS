...
class RegistrosController {
  ...
  public async nuevo(req: Request, res: Response) {
    ...
    try {
      const habitacion_hotel: any = await pool.query(
        "SELECT id, cantidad, disponible, precio FROM habitacion_hotel WHERE fk_id_tipoHabitacion = ? and fk_id_hotel = ?",
        [fk_id_tipoHabitacion, fk_id_hotel]
      );
      ...
      const registroInsertado: any = await pool.query(
        "INSERT INTO registro_huesped (fecha_ingreso, fecha_salida, ciudad_huesped, costo_estancia, fk_id_habitacion_hotel, fk_id_usuario, fk_id_motivo) values (?,?,?,?,?,?,?)",
        [
          fecha_ingreso,
          fecha_salida,
          ciudad_huesped,
          costo,
          habitacion_hotel[0][0].id,
          fk_id_usuario,
          fk_id_motivo,
        ]
      );
  ...
  /**
   * Para traer los registros de HOY del hotel indicado
   * @param req
   * @param res
   * @returns
   */
  public async getRegistros(req: Request, res: Response) {
    ...
    let hoy: any = new Date(); //Solo se pueden ver los registros de hoy
    hoy =
      hoy.getFullYear() +
      "-" +
    ...
    let query =  "SELECT rh.id, rh.fecha_ingreso, rh.fecha_salida, rh.ciudad_huesped, rh.costo_estancia, rh.fk_id_habitacion_hotel, rh.fk_id_usuario, rh.fk_id_motivo, mv.motivo, th.tipo_habitacion FROM registro_huesped rh LEFT JOIN motivo_visita as mv on mv.id = rh.fk_id_motivo LEFT JOIN habitacion_hotel as hh on hh.id = rh.fk_id_habitacion_hotel LEFT JOIN tipo_habitacion as th on th.id = hh.fk_id_tipoHabitacion  WHERE hh.fk_id_hotel = " + idHotel +" AND creado = '" + hoy + "' order by rh.fecha_ingreso";
    const listRegistros = await pool.query(
     query
    );
    /* console.log("hoy: " + hoy);
    console.log(": " + query);
    console.log(listRegistros[0]); */
  ...
  public async actualizar(req: Request, res: Response) {
    ...
    try {
      ...
      await pool.query(
        "UPDATE registro_huesped SET fecha_ingreso = ?, fecha_salida = ?, ciudad_huesped = ?, costo_estancia = ?, fk_id_habitacion_hotel = ?, fk_id_usuario = ?, fk_id_motivo = ? WHERE id = ?",
        [
          fecha_ingreso,
          fecha_salida,
          ciudad_huesped,
      ...
      return res
        .status(401)
        .json({ msg: "Hubo un problema al actualizar los datos.", error });
    }
  }
}
